...
普通管理账户使用后台预置的中国大陆手机号和短信验证码登录，不开放自助注册。生产环境默认使用腾讯云短信，需配置 `TENCENTCLOUD_SECRET_ID`、`TENCENTCLOUD_SECRET_KEY`、`TENCENT_SMS_REGION`、`TENCENT_SMS_SDK_APP_ID`、`TENCENT_SMS_SIGN_NAME` 和 `TENCENT_SMS_TEMPLATE_ID`；`SMS_PROVIDER=fixed` 与 `DEV_SMS_FIXED_CODE` 仅允许 `APP_ENV=development`。本地应急网页登录默认关闭，只在短信服务故障恢复或隔离开发环境显式设置 `APP_LOCAL_LOGIN_ENABLED=true`。应用自身监听普通 HTTP；`APP_BASE_URL` 应填写浏览器实际访问的 origin，Cookie 的 `Secure` 属性跟随其 scheme。只有 localhost 或回环 IP 可使用 `http://`；生产由网关终止 TLS 时，`APP_BASE_URL` 仍填写外部 `https://` 地址。
