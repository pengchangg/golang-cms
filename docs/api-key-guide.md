...
配置管理用于维护客户端可读取的非秘密业务配置，例如功能开关、展示文案、数值阈值、素材引用和已发布内容引用。不要保存密码、Token、私钥、对象存储凭据或其他秘密；配置值会通过内容 API 返回给持有对应 API Key 的客户端。

API Key 的配置访问范围由 `config_namespace_ids` 限制，并与模型 scope 分开校验：读取配置命名空间必须位于 Key 的配置 scope；配置值若引用内容条目，目标模型还必须位于同一个 Key 的模型 scope。当前创建 API Key 仍要求至少授权一个内容模型，配置 scope 可以为空。未授权、已归档或不存在的命名空间统一返回 `404 published_configuration_not_found`，避免泄露资源是否存在。

按稳定 `namespace_key` 一次读取命名空间内全部当前发布配置：

```bash
NAMESPACE_KEY='website'

curl --fail-with-body \
  --header "Authorization: Bearer $API_KEY" \
  "$BASE_URL/api/content/v1/configurations/$NAMESPACE_KEY"
```

响应直接是以 `item_key` 为键、已展开配置值为属性值的 object，适合客户端直接消费完整配置快照：

```json
{
  "maintenance_enabled": false,
  "support_phone": "400-0000"
}
```

只读取一个配置项时，在路径末尾追加稳定 `item_key`：

```bash
ITEM_KEY='maintenance_enabled'

curl --fail-with-body \
  --header "Authorization: Bearer $API_KEY" \
  "$BASE_URL/api/content/v1/configurations/$NAMESPACE_KEY/$ITEM_KEY"
```

两个配置读取端点都只返回当前发布 Revision，不接受查询参数，并支持与内容读取相同的 `ETag`、`If-None-Match`、`304`、限流和过载处理。全量命名空间响应展开后超过 8 MiB、或单项响应超过运行时上限时返回 `413 configuration_response_too_large`。
...
六个 JSON 内容与配置端点成功时返回强 `ETag` 和 `Cache-Control: private, no-cache`。客户端可缓存响应体，并用 `If-None-Match` 验证：
...
| 404 | `published_configuration_not_found` | 配置命名空间或配置项不存在、未发布、已归档、不在 Key scope，或展开目标不可用 |
| 413 | `configuration_response_too_large` | 展开后的配置响应超过运行时上限 |
