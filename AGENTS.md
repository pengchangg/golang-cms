...
- 应急管理员是脱离 RBAC 的 break-glass 身份，动态拥有全部权限且不受角色限制；内置 `high_risk_admin` 角色同样动态全权限并可在非应急用户间传播，但不可编辑、删除或管理任何应急管理员。身份能力必须由服务端主体标记判断，不能从普通权限码反推。
...
- Schema 与素材事务一致性门禁：先准备已完成迁移的专用 MySQL 8.0 测试库，再分别执行 `CMS_TEST_MYSQL_DSN='user:password@tcp(127.0.0.1:3306)/test_db' make test-schema-tx` 和 `CMS_TEST_MYSQL_DSN='user:password@tcp(127.0.0.1:3306)/test_db' make test-asset-tx`；普通 `go test ./...` 未设置该变量时会跳过真实 MySQL 用例。
...
- 普通管理账户使用后台预置的大陆手机号短信验证码登录；生产使用腾讯云短信。`SMS_PROVIDER=fixed` 和 `DEV_SMS_FIXED_CODE` 仅允许 `APP_ENV=development`，且 Base URL 和监听地址必须都是回环地址；应急管理员网页登录还需显式 `APP_LOCAL_LOGIN_ENABLED=true`。完整变量和值域以 `.env.example` 和 `internal/platform/config/config.go` 为准，程序不会自动加载 `.env`。
...
- 远程 MySQL TCP 默认拒绝明文连接；使用 DSN TLS。`MYSQL_ALLOW_INSECURE=true` 仅用于明确隔离的开发网络，production 的 serve、migrate 和 admin 命令均禁止启用。
- 反向代理部署必须通过 `APP_TRUSTED_PROXY_CIDRS` 显式声明直接代理网段；只有直接对端可信时才解析 `X-Forwarded-For` 或单值 `X-Real-IP`，认证限流和审计统一使用同一解析结果。
