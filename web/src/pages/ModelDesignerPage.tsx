...
export default function ModelDesignerPage({ principal }: { principal: Principal }) {
  ...
  const canCreateField = hasSystemPermission(principal, 'models.create')
  ...
  return <><PageHeader eyebrow="字段设计器" title={model.data?.display_name ?? '模型字段'} description="受控添加字段，嵌套对象沿用冻结字段 DTO，最大深度由服务端校验。" extra={<Button type="primary" disabled={!canCreateField || model.data?.status === 'archived'} onClick={() => setOpen(true)}>添加字段</Button>} /><DataState loading={model.loading} error={model.error} empty={!model.data?.fields.length} retry={model.reload}><div className="field-canvas">{model.data?.fields.map((field) => <FieldCard key={field.id} field={field} depth={0} />)}</div></DataState><Modal title="添加根字段" open={open} onCancel={() => setOpen(false)} onOk={createField}><Form form={form} layout="vertical"><Form.Item name="key" label="稳定标识" rules={[{ required: true }, { pattern: /^[a-z][a-z0-9_]{0,63}$/ }]}><Input /></Form.Item><Form.Item name="display_name" label="显示名称" rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="type" label="字段类型" rules={[{ required: true }]}><Select optionLabelProp="label" options={fieldTypeOptions.map((option) => ({ ...option, label: option.label, searchText: `${option.label} ${option.description} ${option.value}` }))} optionRender={(option) => <div className="field-type-option"><strong>{option.data.label}</strong><span>{option.data.description}</span></div>} showSearch optionFilterProp="searchText" /></Form.Item><Form.Item name="required" valuePropName="checked"><Checkbox>必填</Checkbox></Form.Item>{type === 'single_select' || type === 'multi_select' ? <Form.Item name="options" label="选项值，每行一个" rules={[{ required: true }]}><Input.TextArea /></Form.Item> : null}{type === 'single_relation' || type === 'multi_relation' ? <>{targetModels.error ? <Alert type="error" showIcon title="目标模型加载失败" action={<Button size="small" onClick={() => targetModels.reload()}>重试</Button>} /> : null}<Form.Item name="target_model_id" label="目标模型" rules={[{ required: true }]}><Select loading={targetModels.loading} disabled={Boolean(targetModels.error)} showSearch optionFilterProp="searchText" placeholder={relationOptions.length ? '选择其他活动模型' : '暂无其他活动模型'} options={relationOptions} optionRender={(option) => <div className="field-type-option"><strong>{option.data.label}</strong><span>{option.data.key}</span></div>} /></Form.Item></> : null}{type === 'object' || type === 'repeatable_group' ? <Form.Item name="children_json" label="子字段数组 JSON" rules={[{ required: true }]}><Input.TextArea rows={6} placeholder='[{"key":"name","display_name":"名称","type":"single_line_text"}]' /></Form.Item> : null}{type === 'rich_text' ? <Alert type="info" showIcon title="富文本使用结构化 JSON" description="不接收 HTML 字符串。" /> : null}</Form></Modal></>
}
