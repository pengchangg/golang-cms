...
export default function APIKeysPage({ principal }: { principal: Principal }) {
  ...
  async function revoke(row: APIKey) {
    Modal.confirm({ title: `撤销 ${row.name}？`, content: '撤销立即生效且不可恢复。', okText: '确认撤销', okButtonProps: { danger: true }, onOk: async () => { try { await api.revokeAPIKey(row.id); message.success('API Key 已撤销'); keys.reload() } catch (error) { message.error(apiErrorMessage(error, '撤销 API Key 失败')); throw error } } })
  }
  async function rotate(row: APIKey) {
    Modal.confirm({ title: `轮换 ${row.name}？`, content: '新 Key 继承当前名称、模型范围和过期时间，旧 Key 会立即失效。', okText: '确认轮换', onOk: async () => { try { showSecret(await api.rotateAPIKey(row.id)) } catch (error) { message.error(apiErrorMessage(error, '轮换 API Key 失败')); throw error } } })
  }
