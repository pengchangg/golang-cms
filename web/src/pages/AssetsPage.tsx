...
export default function AssetsPage({ principal }: { principal: Principal }) {
  ...
  async function discard(asset: Asset) {
    setDiscardingID(asset.id)
    try {
      await api.discardQuarantinedAsset(asset.id)
      message.success('待确认素材已废弃')
      assets.reload()
    } catch (error) {
      message.error(apiErrorMessage(error, '废弃待确认素材失败'))
      throw error
    } finally {
      setDiscardingID('')
    }
  ...
  return <><PageHeader eyebrow="素材工作区" title="素材库" description="浏览器直传私有对象存储，确认完整性后才允许进入内容版本。" extra={<Button type="primary" disabled={!canUpload} onClick={() => setUploadOpen(true)}>上传素材</Button>} /><PendingApiNotice />
    ...
    <DataState loading={assets.loading} error={assets.error} empty={!assets.data?.items.length} retry={assets.reload}><Table<Asset> rowKey="id" dataSource={assets.data?.items} pagination={false} scroll={{ x: 820 }} columns={[
      ...
      { title: '操作', render: (_, asset) => <Space><Button type="link" href={adminDownloadUrl(`/assets/${encodeURIComponent(asset.id)}/download`)} disabled={asset.status === 'quarantined'}>下载</Button>{asset.status === 'quarantined' ? <><Button type="link" disabled={!canUpload || discardingID === asset.id} loading={confirmingID === asset.id} onClick={() => Modal.confirm({ title: `确认 ${asset.filename}？`, content: '将重新核对对象大小、类型和 SHA-256，校验通过后素材才可被内容引用。', okText: '确认可用', onOk: () => confirm(asset) })}>确认可用</Button><Button type="link" danger disabled={!canArchive || confirmingID === asset.id} loading={discardingID === asset.id} onClick={() => Modal.confirm({ title: `废弃 ${asset.filename}？`, content: '将永久删除尚未确认的上传对象和素材记录，此操作不可恢复。', okText: '废弃', okButtonProps: { danger: true }, onOk: () => discard(asset) })}>废弃</Button></> : null}{asset.status === 'available' ? <Button type="link" danger disabled={!canArchive} onClick={() => Modal.confirm({ title: `归档 ${asset.filename}？`, content: '不可恢复，但不会破坏已有线上引用。', okText: '归档', okButtonProps: { danger: true }, onOk: () => archive(asset) })}>归档</Button> : null}</Space> },
    ]} /></DataState>
