...
const role: Role = { id: 'rol_1', key: 'reader', kind: 'custom', display_name: '只读角色', description: '', system_permissions: [], model_permissions: [{ model_id: 'mdl_1', permissions: ['content.view'] }], created_at: now, updated_at: now }
...
const principal = (permissions: SystemPermission[], readableModels: string[] = []): Principal => ({ user_id: 'usr_1', display_name: '管理员', email: null, auth_method: 'local', is_emergency_admin: false, has_high_risk_role: false, system_permissions: permissions, model_permissions: readableModels.map((model_id) => ({ model_id, permissions: ['content.view'] })) })
...
it('只有 api_keys.create 时可录入模型 ID 创建', async () => {
  ...
  await waitFor(() => expect(createKey).toHaveBeenCalledWith({ name: '新客户端', model_ids: ['mdl_1'], expires_at: null }))
