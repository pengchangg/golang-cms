...
export default function ModelsPage({ principal }: { principal: Principal }) {
  ...
  ]} /></DataState><Modal title="新建内容模型" open={open} onCancel={() => setOpen(false)} onOk={() => form.validateFields().then(api.createModel).then(() => { setOpen(false); form.resetFields(); models.reload() })}><Form form={form} layout="vertical"><Form.Item label="稳定标识" name="key" rules={[{ required: true }, { pattern: /^[a-z][a-z0-9_]{0,63}$/, message: '仅限小写字母、数字和下划线' }]}><Input /></Form.Item><Form.Item label="显示名称" name="display_name" rules={[{ required: true }]}><Input /></Form.Item><Form.Item label="说明" name="description"><Input.TextArea /></Form.Item></Form></Modal></>
}
