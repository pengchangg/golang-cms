...
const event: AuditEvent = { id: 'evt_1', actor_type: 'system', actor_id: null, actor_display_name: null, action: 'session_created', resource_type: 'session', resource_id: null, request_id: 'req_1', ip: '127.0.0.1', user_agent: 'test', result: 'success', failure_code: null, changes: { note: 'ok' }, occurred_at: '2026-07-22T00:00:00Z' }
...
it('点击审计日志行以弹窗展示事件详情', async () => {
  vi.spyOn(api, 'listAuditEvents').mockResolvedValue({ items: [event], next_cursor: null })
  render(<MemoryRouter><AuditPage principal={principal} /></MemoryRouter>)
  await userEvent.click(await screen.findByText('系统'))
  const dialog = await screen.findByRole('dialog')
  expect(within(dialog).getByText('事件详情')).toBeTruthy()
  expect(within(dialog).getByText('req_1')).toBeTruthy()
  expect(within(dialog).getByText(/"note": "ok"/)).toBeTruthy()
})
