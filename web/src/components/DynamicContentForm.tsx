...
function RepeatableGroupEditor({ field, value, onChange, disabled, canSelectAssets, canUploadAssets, referencedAssets, canViewModel, onFieldValidityChange, path }: { field: ContentField; value: unknown; onChange: (value: Record<string, unknown>[]) => void; disabled: boolean; canSelectAssets: boolean; canUploadAssets: boolean; referencedAssets: Record<string, ReferencedAsset>; canViewModel: (modelId: string) => boolean; onFieldValidityChange?: (path: string, valid: boolean) => void; path: string }) {
  ...
  return <div className="repeatable-group"><Space direction="vertical" size="middle" style={{ width: '100%' }}>{items.map((item, index) => <Card key={index} size="small" title={`第 ${index + 1} 项`} extra={<Button size="small" danger disabled={disabled} onClick={() => onChange(items.filter((_, itemIndex) => itemIndex !== index))}>删除</Button>}><DynamicContentForm fields={field.children} content={item} onChange={(next) => onChange(items.map((current, itemIndex) => itemIndex === index ? next : current))} disabled={disabled} canSelectAssets={canSelectAssets} canUploadAssets={canUploadAssets} referencedAssets={referencedAssets} canViewModel={canViewModel} onFieldValidityChange={onFieldValidityChange} path={`${path}/${index}`} /></Card>)}</Space><Button className="repeatable-add" disabled={disabled} onClick={() => onChange([...items, {}])}>添加一项</Button></div>
}
...
export function DynamicContentForm({ fields, content, onChange, disabled = false, canSelectAssets = true, canUploadAssets = false, referencedAssets = {}, canViewModel = () => true, onFieldValidityChange, path = '' }: { fields: ContentField[]; content: Record<string, unknown>; onChange: (value: Record<string, unknown>) => void; disabled?: boolean; canSelectAssets?: boolean; canUploadAssets?: boolean; referencedAssets?: Record<string, ReferencedAsset>; canViewModel?: (modelId: string) => boolean; onFieldValidityChange?: (path: string, valid: boolean) => void; path?: string }) {
  ...
  return <div className="dynamic-form">{fields.filter((field) => field.status === 'active').map((field) => {
    ...
    if (field.type === 'single_media') {
      control = ASSETS_ENABLED ? <AssetPicker multiple={false} value={typeof value === 'string' ? value : null} onChange={(next) => update(field.key, next)} disabled={disabled || !canSelectAssets} canUpload={canUploadAssets} knownAssets={referencedAssets} /> : null
    } else if (field.type === 'multi_media') {
      control = ASSETS_ENABLED ? <AssetPicker multiple value={Array.isArray(value) ? [...new Set(value.filter((item): item is string => typeof item === 'string'))].slice(0, 50) : []} onChange={(next) => update(field.key, next)} disabled={disabled || !canSelectAssets} canUpload={canUploadAssets} knownAssets={referencedAssets} /> : null
    } else if (field.type === 'single_relation') {
      control = <RelationPicker key={`${field.constraints.target_model_id}:${canViewModel(field.constraints.target_model_id ?? '')}`} multiple={false} value={typeof value === 'string' ? value : null} onChange={(next) => update(field.key, next)} disabled={disabled} targetModelId={field.constraints.target_model_id} canViewTarget={Boolean(field.constraints.target_model_id && canViewModel(field.constraints.target_model_id))} />
    ...
    else if (field.type === 'rich_text') control = <Suspense fallback={<div className="editor-loading">正在加载富文本编辑器</div>}><RichTextEditor label={field.display_name} value={value} onChange={(next) => update(field.key, next)} disabled={disabled} /></Suspense>
    else if (field.type === 'json') control = <Suspense fallback={<div className="editor-loading">正在加载 JSON 编辑器</div>}><JsonEditor label={field.display_name} value={value} onChange={(next) => update(field.key, next)} disabled={disabled} onValidityChange={(valid) => onFieldValidityChange?.(`${path}/${field.id}`, valid)} /></Suspense>
    else if (field.type === 'object') control = <DynamicContentForm fields={field.children} content={value && typeof value === 'object' && !Array.isArray(value) ? value as Record<string, unknown> : {}} onChange={(next) => update(field.key, next)} disabled={disabled} canSelectAssets={canSelectAssets} canUploadAssets={canUploadAssets} referencedAssets={referencedAssets} canViewModel={canViewModel} onFieldValidityChange={onFieldValidityChange} path={`${path}/${field.id}`} />
