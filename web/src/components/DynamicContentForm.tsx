...
function RepeatableGroupEditor({ field, value, onChange, disabled, canSelectAssets, canUploadAssets, referencedAssets }: { field: ContentField; value: unknown; onChange: (value: Record<string, unknown>[]) => void; disabled: boolean; canSelectAssets: boolean; canUploadAssets: boolean; referencedAssets: Record<string, ReferencedAsset> }) {
  ...
  return <div className="repeatable-group"><Space direction="vertical" size="middle" style={{ width: '100%' }}>{items.map((item, index) => <Card key={index} size="small" title={`第 ${index + 1} 项`} extra={<Button size="small" danger disabled={disabled} onClick={() => onChange(items.filter((_, itemIndex) => itemIndex !== index))}>删除</Button>}><DynamicContentForm fields={field.children} content={item} onChange={(next) => onChange(items.map((current, itemIndex) => itemIndex === index ? next : current))} disabled={disabled} canSelectAssets={canSelectAssets} canUploadAssets={canUploadAssets} referencedAssets={referencedAssets} /></Card>)}</Space><Button className="repeatable-add" disabled={disabled} onClick={() => onChange([...items, {}])}>添加一项</Button></div>
}

export function DynamicContentForm({ fields, content, onChange, disabled = false, canSelectAssets = true, canUploadAssets = false, referencedAssets = {} }: { fields: ContentField[]; content: Record<string, unknown>; onChange: (value: Record<string, unknown>) => void; disabled?: boolean; canSelectAssets?: boolean; canUploadAssets?: boolean; referencedAssets?: Record<string, ReferencedAsset> }) {
  ...
  return <div className="dynamic-form">{fields.filter((field) => field.status === 'active').map((field) => {
    ...
    else if (field.type === 'object') control = <DynamicContentForm fields={field.children} content={value && typeof value === 'object' && !Array.isArray(value) ? value as Record<string, unknown> : {}} onChange={(next) => update(field.key, next)} disabled={disabled} canSelectAssets={canSelectAssets} canUploadAssets={canUploadAssets} referencedAssets={referencedAssets} />
    else if (field.type === 'repeatable_group') control = <RepeatableGroupEditor field={field} value={value} onChange={(next) => update(field.key, next)} disabled={disabled} canSelectAssets={canSelectAssets} canUploadAssets={canUploadAssets} referencedAssets={referencedAssets} />
